...
export default class Tabs extends Component {
  ...
  __onFocusMove(e) {
    ...
    switch (e.keyCode) {
      case KEYCODE_LEFT:
        if (currIndex !== 0) {
          this.updateIndex(currIndex - 1, this.tabRefs[`tab${currIndex - 1}`].getAttribute('data-label').toLowerCase());
        }
        break;
      case KEYCODE_RIGHT:
        ...
        ${this.props.children.map((child, i) => {
          return html`
            <li role="presentation" key=${i} class="${i > 1 ? 'hide' : ''}">
              ...
              >
                ${child.props.id === 'settings' && this.state.updateAvailable ? html` <div class="ld-menu--notification"></div> ` : ''}
                ${ICONS[child.props.id]}
                <p class="${index === i ? 'ld-menu--active' : ''}">${child.props.id.toUpperCase()}</p>
              </button>
            </li>
          `;
        })}
